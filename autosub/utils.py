...
def extract_audio(path: StrOrPath) -> NDArray:
    with tempfile.TemporaryDirectory() as dir:
        ...
        ffmpeg.input(str(path)).output(output_path.as_posix(), acodec='pcm_s16le',
                                 ac=1, ar='16k').run(quiet=True, overwrite_output=True)
        audio = whisper.load_audio(output_path.as_posix())
    return audio


def transcribe(audio: NDArray, output_path: StrOrPath, transcribe_func: Callable[[NDArray], Dict]):
...
def embed_subtitle(video_path: StrOrPath, srt_path: StrOrPath, output_path: StrOrPath):
    ...
    audio = video.audio
    ffmpeg.concat(video.filter('subtitles', str(srt_path), force_style='OutlineColour=&H40000000,BorderStyle=3'),
                  audio, v=1, a=1).output(str(output_path)).run(quiet=True, overwrite_output=True)
