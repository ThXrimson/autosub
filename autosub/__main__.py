...
def main():
    ...
    transcribe_group.add_argument('--overwirte', action='store_true',
                                  default=False, help='whether to overwrite the existing files')
    ...
    overwrite = opts.overwirte
    embed = opts.embed

    autosub(inputs, output_dir, model, gpu, task,
            language, simplified, initial_prompt, overwrite, verbose, embed)
